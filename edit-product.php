    ...
    if (isset($_POST['update'])) {
        ...
        if ($_FILES["image"]["error"] == UPLOAD_ERR_NO_FILE) {
            if (!empty($_POST['expirydate'])) {
                $query = mysqli_query($con, "update tblproducts set CategoryName='$catname',CompanyName='$company',ProductName='$pname',ProductPrice='$pprice',ExpiryDate='$exdate',Stock='$stock' where id='$pid'");
                echo "<script>alert('Product updated successfully.');</script>";
                echo "<script>window.location.href='manage-products.php'</script>";
            } else {
                $query = mysqli_query($con, "update tblproducts set CategoryName='$catname',CompanyName='$company',ProductName='$pname',ProductPrice='$pprice',Stock='$stock' where id='$pid'");
                echo "<script>alert('Product updated successfully.');</script>";
                echo "<script>window.location.href='manage-products.php'</script>";
            }
        } else {
            $tempFile = $_FILES['image']['tmp_name'];
            $newLocation = 'img/' . $_FILES['image']['name'];
            move_uploaded_file($tempFile, $newLocation);
            if (!empty($_POST['expirydate'])) {
                $query = mysqli_query($con, "UPDATE tblproducts SET CategoryName='$catname',CompanyName='$company',ProductName='$pname',ProductPrice='$pprice',ExpiryDate='$exdate',Stock='$stock',ImagePath='$newLocation' WHERE id='$pid'");
                echo "<script>alert('Product updated successfully.');</script>";
                echo "<script>window.location.href='manage-products.php'</script>";
            } else {
                $query = mysqli_query($con, "UPDATE tblproducts SET CategoryName='$catname',CompanyName='$company',ProductName='$pname',ProductPrice='$pprice',Stock='$stock',ImagePath='$newLocation' WHERE id='$pid'");
                echo "<script>alert('Product updated successfully.');</script>";
                echo "<script>window.location.href='manage-products.php'</script>";
            }
        }
                                            ...
                                            ?>
                                                <div class="form-row">
                                                    <div class="col-md-6 mb-10">
                                                        ...
                                                        <select class="form-control custom-select" name="category" required>
                                                            ...
                                                            <?php
                                                            $ret = mysqli_query($con, "select CategoryName from tblcategory where userID='$uid' ");
                                                ...
                                                <div class="form-row">
                                                    <div class="col-md-6 mb-10">
                                                        ...
                                                        <select class="form-control custom-select" name="company" required>
                                                            ...
                                                            <?php
                                                            $ret = mysqli_query($con, "select CompanyName from tblcompany  where userID='$uid' ");
                                                ...
                                                <div class="form-row">
                                                    <div class="col-md-6 mb-10">
                                                        ...
                                                        <br />
                                                        <img style="width: 300px;" src="<?php echo $result['ImagePath']; ?>" />
                                                        <input type="file" class="form-control" id="validationCustom03" name="image">
                                                        <div class="invalid-feedback">Please provide a valid product image.</div>
                                                    </div>
                                                </div>
