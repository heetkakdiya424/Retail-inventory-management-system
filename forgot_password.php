...
if (isset($_POST['send'])) {
    ...
    if ($ret > 0) {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'falgunforvp@outlook.com';
            $mail->Password = 'Tiger@1122';
            $mail->SMTPSecure = 'STARTTLS'; // Enable TLS encryption, 'ssl' also accepted
            $mail->Port = 587; // TCP port to connect to

            // Recipients
            $mail->setFrom('falgunforvp@outlook.com', 'EasyTex Mail Services');
...
if (isset($_POST['submit'])) {
    if ($_POST['cpassword'] != $_POST['password']) {
        ...
        if (preg_match($pattern, $_POST['password'])) {
            ...
            $query = mysqli_query($con, "update tbladmin set Password='$newpassword' where id='$uid' ");
