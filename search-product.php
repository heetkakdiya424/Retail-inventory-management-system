    ...
    if (isset($_POST['checkout'])) {
        ...
        if (preg_match($pattern, $cmobileno) and preg_match($patternCN, $cname)) {
            ...
            foreach ($value as $pdid => $qty) {
                $uopstock = 0;
                $query = mysqli_query($con, "insert into tblorders(userID,ProductId,Quantity,InvoiceNumber,CustomerName,CustomerContactNo,PaymentMode) values('$userID','$pdid','$qty','$invoiceno','$cname','$cmobileno','$pmode')");
                $getstock = mysqli_query($con, "SELECT Stock FROM tblproducts WHERE id='$pdid'");
                $productByStock = mysqli_fetch_array($getstock);
                $stock = $productByStock['Stock'];
                $upstock = intval($stock) - intval($qty);
                $uppro = mysqli_query($con, "update tblproducts set Stock='$upstock' where id='$pdid'");
            }
                                            ...
                                            <table id="datable_1" class="table table-hover w-100 display pb-30">
                                                ...
                                                <tbody>
                                                    <form method="POST" action="test.php">
                                                        ...
                                                        $query = mysqli_query($con, "select * from tblproducts where userID='$userID'");
                                                        $cnt = 1;
                                            ...
                                            <div class="table-wrap">
                                                <table id="datable_1" class="table table-hover w-100 display pb-30">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        $query = mysqli_query($con, "select * from tblproducts where ProductName like '%$pname%' and userID='$userID'");
                                                        $cnt = 1;
                                                        while ($row = mysqli_fetch_array($query)) {
                                                        ?>
